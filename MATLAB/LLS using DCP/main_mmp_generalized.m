...
% plot comparing 1/T for W-FIR and W-IIR
n_fir = [12, 1, 3];
n_iir = [13, 2, 4];
figure
    hold on
    for i = 1:3
        h(i) = semilogx(w_in_Hz,mag(n_fir(i),:));
        h(i).LineStyle = line_style{i};
        h(i).LineWidth = 1.5;
    end
    % x_line = xline(Nyq_Hz);
...
fprintf('FIR min |1/T|: SOCP: %.5f, SDP:FIR %.5f, SDP:IIR %.5f\n', h_inf(n_fir));
fprintf('IIR min |1/T|: SOCP: %.4f, SDP:FIR %.4f, SDP:IIR %.4f\n', h_inf(n_iir));
%%
w_rad_dist = w_d/T_fs;
mag_dist = zeros(2, length(n_fir), length(w_d));
for i = 1:length(n_fir)
    [mag, ~, ~] = bode(Tinv(n_fir(i)),w_rad_dist);
    mag_dist(1,i,:) = squeeze(mag);
    [mag, ~, ~] = bode(Tinv(n_iir(i)),w_rad_dist);
    mag_dist(2,i,:) = squeeze(mag);
end
mag_dist
